...
CREATE TABLE orders (
    ...
    payment_proof_url VARCHAR(255),                -- screenshot/photo of payment receipt
    notes             TEXT,
...
--   category is an ENUM so reports never break on typos.
--   currency mirrors gift_cards; totals are grouped by currency
--   because summing across currencies is meaningless. Store a
--   normalized amount_base if you want one consolidated total.
--   expense_date = day the cost was incurred (key for monthly
--   reports), distinct from created_at (when the row was entered).
-- ------------------------------------------------------------
CREATE TABLE expenses (
    id            INT AUTO_INCREMENT PRIMARY KEY,
    category      ENUM('COGS','Payment Gateway Fees','Marketing','Hosting','Other') NOT NULL,
...
-- ------------------------------------------------------------
CREATE INDEX idx_expenses_month_cat ON expenses (expense_date, category);
